...
int digitDp_Util(string &s, int i, int sum, int tight) {
...
void solve() {
    lli L, R, sum; cin >> L >> R >> sum;
    cout << (getForAllSum(to_string(R), 0, sum) - 
            getForAllSum(to_string(L-1), 0, sum) + MOD) % MOD << endl;
}   
