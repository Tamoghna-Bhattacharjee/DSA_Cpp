...
lli getDist(int L, int R) {
    if (L == R) return 0;
    lli mi = min(a[L], a[R]), mx = max(a[L], a[R]);
    node t = st.qry(L,R);
    if (t.mi == mi && t.mx == mx) return 1;
    else if (t.mi == mi) return getDist(L, p[t.mx]) + getDist(p[t.mx], R);
    else return getDist(L, p[t.mi]) + getDist(p[t.mi], R);
}
