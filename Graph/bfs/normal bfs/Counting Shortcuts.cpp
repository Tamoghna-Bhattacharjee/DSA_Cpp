#include <bits/stdc++.h>
// #include <ext/pb_ds/assoc_container.hpp>
// #include <ext/pb_ds/tree_policy.hpp>
// using namespace __gnu_pbds;
using namespace std;

#define lli long long int 
#define ld long double
#define vi vector<int>
#define vlli vector<lli>
#define vpii vector<pair<int, int>>
#define pb push_back 

template<typename T> void debug(T _a) {cout << _a << " ";}
template<typename T1, typename T2> void debug(pair<T1, T2> _p) {cout<<"{";debug(_p.first);cout<<": ";debug(_p.second);cout<<"}\n";}
template<typename T> void debug(vector<T> _aa) {for (auto h: _aa) debug(h); cout << endl;}
template<typename T> void debug(deque<T> _aa) {for (auto h: _aa) debug(h); cout << endl;}
template<typename T> void debug(multiset<T> _aa) {for (auto h: _aa) debug(h); cout << endl;}
template<typename T> void debug(set<T> _aa) {for (auto h: _aa) debug(h); cout << endl;}
template<typename T1, typename T2> void debug(map<T1, T2> _mm) {for (auto h: _mm) debug(h);}

// typedef tree<int, null_type, less<int>, 
//             rb_tree_tag, tree_order_statistics_node_update> ordered_set;

const lli INF = 1e18, MOD = 1e9 + 7;
const int N = 2e5;
const int di[] = {-1,0,1,0}, dj[] = {0,1,0,-1};
const string YN[] = {"NO", "YES"};

// https://codeforces.com/contest/1650/problem/G

void solve() {
    int n, m, s, t; cin >> n >> m >> s >> t;
    vi g[n+1];
    for (int i = 0; i < m; i++) {
        int u, v; cin >> u >> v;
        g[u].pb(v); g[v].pb(u);
    }
    queue<int> q; q.push(s); 
    vlli dist(n+1, INF); dist[s] = 0;
    while (!q.empty()) {
        int u = q.front(); q.pop();
        int d = dist[u];
        for (int v: g[u]) {
            if (d+1 < dist[v]) dist[v] = d+1, q.push(v);
        }
    }
    vector<vector<lli>> dp(n+1, vector<lli>(2, 0));
    vector<vi> visit(n+1, vi(2, false)); 
    dp[s][0] = 1;
    queue<pair<int, int>> qq;
    qq.push({s, 0}); // [node, type]
    while (!qq.empty()) {
        auto cur = qq.front(); qq.pop();
        int u = cur.first, ty = cur.second;
        if (visit[u][ty]) continue;
        visit[u][ty] = true;
        for (int v: g[u]) {
            if (ty == 0) {
                if (dist[u] + 1 == dist[v]) {
                    dp[v][0] += dp[u][0]; dp[v][0] %= MOD;
                    qq.push({v, 0}); 
                }
                if (dist[u] + 1 == dist[v] + 1) {
                    dp[v][1] += dp[u][0]; dp[v][1] %= MOD;
                    qq.push({v,1});
                }
            } else {
                if (dist[u] + 1 == dist[v]) {
                    dp[v][1] += dp[u][1]; dp[v][1] %= MOD;
                    qq.push({v, 1});
                }
            }
        }
    }
    cout << (dp[t][0] + dp[t][1]) % MOD << endl;

}   
  
int main() {
    #ifndef ONLINE_JUDGE
        freopen("input.txt","r",stdin);
        freopen("output.txt","w",stdout);
    #endif
    ios::sync_with_stdio(false);
    cin.tie(NULL); cout.tie(0);
    cout << fixed << setprecision(9);
    int t = 1; cin >> t;
    for (int _i = 1; _i <= t; _i++) {
        //cout << "Case #" << _i << ": ";
        solve();
    }
}