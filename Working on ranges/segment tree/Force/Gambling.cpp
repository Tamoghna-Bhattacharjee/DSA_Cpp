...
void solve() {
    ...
    for (auto i: mp) {
        int x = i.first;
        vi v = i.second;
        for (int j: v) st.rupd(j,j,1);
        int mxSeg = st.qry(1,n).seg;
        if (mxSeg > val) val = mxSeg, ans = x;
        for (int j: v) st.rupd(j,j,-1);
    }
    ...
    ind[0] = 0;
    vi b(n+1, -1); 
    for (int i: mp[ans]) b[i] = 1;
    for (int i = 1; i <= n; i++) {
        sum += b[i];
        if (ind.find(sum-val) != ind.end()) {
            cout << ans << " " << ind[sum-val] + 1 << " " << i << endl;
            return;
        }
        ind[sum] = i;
    }
